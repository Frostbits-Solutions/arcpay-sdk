...
<template>
  <div class="ap-w-full sm:ap-w-96">
    ...
    <button v-if="status?.status === 'active'"
            ...
            @click="emit('action:buy', formatPrice(price, listingParams.currency_decimals || undefined))"
      ...
      <span class="text ap-flex ap-items-center ap-gap-1">
          ...
          <div class="ap-flex ap-items-center">
            <span class="ap-text-3xl ap-font-extrabold ap-tracking-tight">
              <count-up :decimalPlaces="2" :duration="1" :end-val="formatPrice(price, listingParams.currency_decimals || undefined)" :start-val="formatPrice(previousPrice, listingParams.currency_decimals || undefined)"></count-up>
            </span>
