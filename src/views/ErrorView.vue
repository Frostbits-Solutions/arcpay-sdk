    ...
    <div class="ap-flex-1 ap-flex ap-items-end ap-justify-center ap-pb-4">
      <div style="transform: translate(6px, -5px)">
        <svg class="checkmark" viewBox="0 0 52 52" xmlns="http://www.w3.org/2000/svg">
          ...
          <path class="checkmark__check" d="M16 16 36 36 M36 16 16 36" fill="none"/>
        </svg>
      </div>
    </div>
    <div
      ...
      <div class="ap-animate-in ap-slide-in-from-bottom-2 ap-fade-in ap-delay-75 ap-fill-mode-both">
        ...
        <div v-if="args?.description" class="ap-text-xs ap-text-muted-foreground ap-break-all">{{
            args.description
          }}
        </div>
      </div>
      <Button class="ap-mb-8 ap-grow-0 ap-w-24 ap-bg-background" size="lg" variant="secondary" @click="callback">Close
      </Button>
    </div>
  </div>
</template>
